...
class Puzzle(Base):
    def solution(self):
        ...
        for elves in self.input:
            # binary OR comparison so that we get a combination of both elves' sections
            # 0111000 | 0001110   ==  0111110
            overlap = elves[0] | elves[1]

            # if the combined section binary is the same as either of the input binaries
            # then one is completely inside another    1= overlap   0= no overlap
            overlaps.append(1 if overlap == elves[0] or overlap == elves[1] else 0)
