...
class Puzzle(Base):
    def parse_notes(self):
        ...
        # keep track of the monkey test divisors here so we can initialise their remainders
        divisors = []
        ...
        for line in self.input:
            ...
            if items_match is not None:
                self.monkeys[monkey_index]['items'] = [{1: int(i.strip())} for i in items_match.group(1).split(",")]
            ...
            if test_match is not None:
                ...
                divisors.append(self.monkeys[monkey_index]['test'])
        ...
        # loop back through each monkey's items
        for monkey in self.monkeys:
            for item in monkey["items"]:
                # calculate the initial remainder for each divisor
                for div in divisors:
                    item[div] = item[1] % div
    ...
    def solution(self):
        ...
        for round in range(rounds):
            ...
            for monkey in self.monkeys:
                ...
                for item in monkey["items"]:
                    # loop through all of the remainders updating based on this monkey's equasion
                    for _, rem in enumerate(item.items()):
                        item[rem[0]] = (lambda old:eval(monkey["operation"]))(rem[1]) % rem[0]
